...
> EPA ozone measurements for 2018-2019 were significantly lower than 50, the AQI threshold for "good" air quality (t-test; t = -57.98, p < 0.0001). - Kateri
The mean of x is 40.87526, the p-value is less than .0002, which is significant. rejest 
the null hypothesis. we can accept the alternative hypothesis - Monsiha
...
plot(O3.twosample2)
par(mfrow = c(1,1)) # to change the way R will display the plots
```
...
```{r}
#1
plot1 <- ggplot(EPAair, aes(x = Ozone, color = Year)) +
  geom_density(fill = "gray")
print(plot1) # added by me -  Monisha

#2
plot2 <- ggplot(EPAair, aes(x = Ozone, color = Year)) +
  geom_bboxplot()
