...
#monisha - all of the above give the same results
...
# Monisha - the above should give us an idea about what the max and min depths are

# Exercise: 
# Arrange NTL.phys.data by temperature, in descending order. 
# Which dates, lakes, and depths have the highest temperatures?

NTL.phys.data.temp <- arrange(NTL.phys.data, desc(temperature_C)) # Monisha - new addition works for temperature
```
### Select
...
NTL.phys.data.temps <- select(NTL.phys.data, -lakename, sampledate:temperature_C) # Monisha - new addition: excludes lakename
```
### Mutate
...
NTL.phys.data.PeterPaul1 <- mutate(NTL.phys.data.PeterPaul1, month = month(sampledate)) # Monisha -making new column "month" 
