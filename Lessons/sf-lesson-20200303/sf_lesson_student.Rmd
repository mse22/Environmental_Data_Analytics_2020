...
> Shape of earth means map is not a 100% accurate representation
...
# I am getting NA for epsg (SRID) - Monisha
```
...
my_tabular_data_raw <- read.csv(here("data", "tabular_data", "conterm_climate.txt")) # read.csv since it gives regular tabular data that doesnt not have any spatial geometry asscociated with it
...
my_epsg <- 5070  # since they are both refered to the same reference system

# setting projections
st_crs(basins_nf_seplains_raw) <- my_proj4 # asks to use st_transform instead? - Monisha
st_crs(basins_nf_seplains_raw) <- my_epsg  #not changing the data fundamental; just the data stored in R - Monisha
st_crs(basins_nf_seplains_raw)
...
southeast_state_bounds.NC <- 
select(southeast_state_bounds, NC)
